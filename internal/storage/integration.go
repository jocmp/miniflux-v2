...
// Icons returns all icons with HMAC salt
func (s *Storage) GoogleReaderGetIcons() (model.GoogleReaderIcons, error) {
	query := `
		SELECT
			icons.id,
			icons.hash,
	...
	var icons model.GoogleReaderIcons
	for rows.Next() {
		var icon model.GoogleReaderIcon
		err := rows.Scan(&icon.Icon.ID, &icon.Icon.Hash, &icon.Icon.MimeType, &icon.Icon.Content, &icon.Salt)
		if err != nil {
...
func (s *Storage) UpdateIntegration(integration *model.Integration) error {
	query := `
		...
		SET
			...
			googlereader_salt=$27,
			telegram_bot_enabled=$28,
			telegram_bot_token=$29,
			telegram_bot_chat_id=$30,
			telegram_bot_topic_id=$31,
			telegram_bot_disable_web_page_preview=$32,
			telegram_bot_disable_notification=$33,
			telegram_bot_disable_buttons=$34,
			espial_enabled=$35,
			espial_url=$36,
			espial_api_key=$37,
			espial_tags=$38,
			...
			linkace_api_key=$41,
			linkace_tags=$42,
			linkace_is_private=$43,
			linkace_check_disabled=$44,
			linkding_enabled=$45,
			linkding_url=$46,
			linkding_api_key=$47,
			linkding_tags=$48,
			linkding_mark_as_unread=$49,
			matrix_bot_enabled=$50,
			matrix_bot_user=$51,
			matrix_bot_password=$52,
			matrix_bot_url=$53,
			matrix_bot_chat_id=$54,
			notion_enabled=$55,
			notion_token=$56,
			notion_page_id=$57,
			readwise_enabled=$58,
			...
			shaarli_enabled=$72,
			shaarli_url=$73,
			shaarli_api_secret=$74,
			webhook_enabled=$75,
			webhook_url=$76,
			webhook_secret=$77,
			rssbridge_enabled=$78,
			rssbridge_url=$79,
			omnivore_enabled=$80,
			omnivore_api_key=$81,
			omnivore_url=$82,
			linkwarden_enabled=$83,
			linkwarden_url=$84,
			linkwarden_api_key=$85,
			raindrop_enabled=$86,
			raindrop_token=$87,
			raindrop_collection_id=$88,
			raindrop_tags=$89,
			betula_enabled=$90,
			betula_url=$91,
			betula_token=$92,
			...
			ntfy_password=$98,
			ntfy_icon_url=$99,
			ntfy_internal_links=$100,
			cubox_enabled=$101,
			cubox_api_link=$102,
			discord_enabled=$103,
			discord_webhook_link=$104,
			slack_enabled=$105,
			slack_webhook_link=$106,
			pushover_enabled=$107,
			pushover_user=$108,
			pushover_token=$109,
			pushover_device=$110,
			pushover_prefix=$111
